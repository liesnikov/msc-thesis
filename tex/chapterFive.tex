...
Instead, it stores whole proof state in the goal, thus limiting the usability of \coqe{match goal} significantly.
...
The third one binds the expression attached to the resource a to \coqe{c} in case user wants to minuplate it or force its unification with either $\true$ or $\false$ manually.
...
From Ltac2 we take the \coqe{Pattern.matches} function, \coqe{match! goal} tactical and notation (a scope, in Ltac2 terms) for goal pattern-matching.
...
We then compute all possible pairings of hypotheses from the contexts and patterns with help of \coqe{Pattern.matches} and return them as a list alternatives inside monad \coqe{M}.
We also match current MoSeL goal against goal pattern \coqe{pconcl} with \coqe{Pattern.matches}, take a direct product of the result of goal-matching and hypothesis-matching as two monadic lists of values and merge substitution maps.
