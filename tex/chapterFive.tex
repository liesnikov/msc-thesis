...
We also match current IPM goal with goal pattern \coqe{pconcl}, take a direct product of these two monadic lists of values and merge substitution maps.
...
in Control.plus cur rest
\end{coq}
Which in its own turn evaluates to \coqe{rest}, which is equivalent to switching to a different branch from a user perspective.
...
However, Ltac1 has not only \coqe{match}, but also two alternatives: \coqe{lazymatch} and \coqe{multimatch}, both for goal and term matching.
