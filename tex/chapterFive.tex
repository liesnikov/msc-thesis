...
\section{Motivating examples} % just a suggestion

While one of the best illustrative examples for \coqe{iMatch} would be a solver for separation logic, we are going to cover that in chapter~\ref{chap:solver}.
...
A notable thing with this tactic is not just readability, assuming one is familiar with the \coqe{match goal} construct,
...
\item There is also a technical limitation: at the moment it is not possible to implement non-linear patterns in Ltac2, so technically it isn't possible to implement \coqe{i_assumption'} for \emph{pattern-matching} to ensure that the goal and conclusion of the wand match.
...
After taking apart one branch \coqe{ph}, we bind \coqe{rest} to the rest of the computation (matches other branches) and compute the returned value on this branch.
...
We then compute all possible pairings of hypotheses from the contexts and patterns with help of \coqe{Pattern.matches} and return them as a list of alternatives inside monad \coqe{M}.
...
In case \coqe{i_match_patterns_goal} can't match the current proof state with this branch's patterns (\coqe{i_match_patterns_goal phyps pconcl}) or when the right-hand side of the branch fails (\coqe{f hids subst}), \coqe{cur} will be set to \coqe{fun () => Control.zero e}, for some \coqe{e}.
...
In this section we are going to explain what the differences in semantics are between those variants and we will cover some details about the implementation of them for \coqe{iMatch}.
...
An example that doesn't rely on the order and simply uses unification would go as follows.
...
\item \coqe{iLazyMatch} erases backtracking points after any successful hypothesis pairing.
  I.e., once the hypothesis matching phase of \coqe{iLazyMatch} is over, tactic failures inside a branch won't cause any backtracking and the whole expression will return the exception with which the tactic failed.
...
The reason for this is that, while \coqe{r} can be applied to the current goal, there is no resource to provide the wand with after it has been applied.
...
This variant is called \coqe{iMultiMatch} and the name is literally the only change that is necessary in the definition of \coqe{i_assumption''}.
...
One possible workaround to implement non-linear patterns for \coqe{iMatch} would involve simply verifying that names are convertible after we produced an assignment from patterns to hypotheses.
...
\begin{itemize}
\item
  As mentioned above multiple times, perhaps one of the biggest limitations encountered while developing \coqe{iMatch} was the inability to implement non-linear patterns.
...
\item Another aspect of pattern handling that limits usability of \coqe{iMatch} is the lack of \coqe{constr} antiquotation inside patterns.
  ...
  Alternatively, it would allow us to parameterize implementations in terms of entailment predicates, which is currently not possible and we have to maintain two different implementations for \coqe{iMatch}: one for regular MoSeL entailment and one with constraints.
\item While we were able to emulate some kind of separation between contexts inside patterns with \coqe{_ : $\Vert$}, it is again a workaround and not a particularly elegant one.
  ...
  \coqe{goal_matching} scope is clearly too complicated to be expressed with existing scope combinators of Ltac2 and it spans upwards of a dozen of lines in CoqPP syntax.
\item The final issue we encountered while implementing \coqe{iMatch} was also related to separators of contexts.
  Ltac2 provides a module to probe the AST of Coq terms inside Ltac2 -- Constr.Unsafe -- which proved to be invaluable for the implementation of the new \coqe{iSplit} tactic and working with existential variables in general.
