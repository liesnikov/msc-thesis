...
It was later extended to MoSeL\cite{krebbersMoSeLGeneralExtensible2018} by , a general framework for modal separation logics in Coq.
...
\item Then we cover the embedding of logic in Coq and what MoSeL entailments correspond to in BI.
...
It is important, however, that from implementation perspective MoSeL tactics are not just rules that we can \coqe{apply}.
...
Now that we have seen MoSeL from theoretical perspective, let's turn to Coq implementation.
...
Then we can define entailment predicate, which includes not only both contexts, but also a proof that identifiers in each context are unique \coqe{$\ulcorner$ envs_wf $\Delta \urcorner$}.
\begin{coq}
...
Typeclasses serve multiple purposes in MoSeL development, and here we are going to describe only couple of simplest instances needed for \coqe{iAssumption}.
...
\end{coq}
where \coqe{$\intuit$?p} is a technical connective which represent optional modality.
...
The simplest instance of this is just an identity \coqe{P |- P}, which doesn't have to match neither left or right input, so it uses \coqe{FromAssumption} directly.
...
Turning again to \coqe{iAssumption}, the idea is that the user doesn't provide explicit identifier of the resource they have in mind, instead tactic is supposed to find it on its own.
