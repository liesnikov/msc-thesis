...
In order to work with Coq terms effectively, Ltac2 gives uses the ability to match them in a way that is very similar syntactically to matching proper Ltac2 terms: the user has to swap \coqe{match} for \coqe{match!} and prepend variable names bound in patterns with question marks:
\begin{coq}
...
% The quote environments are to fix box overflows
\begin{quote}
\coqe{tactic1 > [tactic2_1 | $\ldots$ | tactic2_n]} which stands for tactic \end{quote}
dispatch and was written as
\begin{quote}
\coqe{tactic1; [tactic2_1 |  $\ldots$ | tactic2_n]}
\end{quote}
in Ltac1.
