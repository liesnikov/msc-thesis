...
First of all, Ltac2 reflects Coq terms into \coqe{constr} datatype and provides a library to work with them.
...
We are going to cover them in section~\ref{sec:semantics-ltac2} in detail, but for it suffices to say that there are three primitives, which are in \coqe{Control} library:
...
As we said, \coqe{zero} throws an exception, \coqe{plus} introduces a backtracking point, which will switch to the second branch if first argument throws an exception.
...
\item We are using a backtracking primitive \coqe{plus} to attempt proving both left and right branches.
  And while the second argument to \coqe{plus} takes exception, that the first branch generated, we simply ignore it.
