...
\subsection{\coqe{iSplit} in action}
...
Everything up to \coqe{i_split} stays the same, both in the proof script and the proof states.
...
The idea is that we combine the constraints for the resulting resource \(\psi\), while ensuring that a user still has the choice to not utilize it.
...
Since then it is possible to simply apply \emph{ex falso} rule and provide \coqe{p} as a proof for \(\bot\) no matter what the expression evaluates to.
...
To explain instantiation of let's take our running example of \coqe{iAssumption}.
