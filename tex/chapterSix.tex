...
\item Performance: while neither Ltac2, nor the solver were developed with execution speed in focus, we have to note that our naive implementation suffers at times from slowness.
  ...
  While it is hard to place blame without proper profiling tools available, experiments suggested that the backtracking through many branches inside \coqe{iMatch} may cause significant slowdown, and how to speed that up, except reimplementation in OCaml, is not clear.
\end{itemize}
