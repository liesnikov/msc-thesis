...
Both of these are done using \coqe{iMatch} to select the right action based on the hypothesis found and the shape of the goal.
...
We then connect (figure~\ref{fig:i-solver}) the two parts with \coqe{orelse} operator, that essentially serves as try-catch expression, executing the second half of the solver only when the first one fails.
...
Despite simplicity, \coqe{i_solver} is able to handle a large fragment of first-order separation logic.
For the sake of simplicity we don't include tactics to work with \coqe{heaplang}, so it would more precise to say that we build a solver for BI logic.
The inclusion of tactics for Hoare triples is possible, however, and doesn't present a major challenge.

At the moment, however, \coqe{i_solver} is able to solve fairly complex MoSeL entailments, including goals with elaborate conjunctions and ones that require deeply nested wand application.
...
\item Modular:
  ...
  Alternatively, it is also possible to incorporate a call to external tactic, similar to the way \coqe{naive_solver} is doing it.
\end{itemize}

However, there are also several downsides that stem from the nature of implementation.
\begin{itemize}
...
\item Performance: while neither Ltac2, nor the solver were developed with execution speed in focus, we have to note that our naive implementation suffers at times from slowness.
  ...
  And while it is hard to place blame without proper profiling tooling available, but experiments suggested that backtracking inside \coqe{iMatch} may cause significant slowdown, andwhat is the way to speed it up, except reimplementation in OCaml, is not clear.
\end{itemize}
